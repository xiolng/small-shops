<template>
	<view class="category-main">
		<view class="u-search-box">
			<view class="u-search-inner" @click="$u.route(`/pages/Goods/GoodsSearch`)">
		...
		<view class="u-menu-wrap">
			...
			<scroll-view :scroll-top="scrollRightTop" scroll-y scroll-with-animation class="right-box" @scroll="rightScroll">
				<view class="page-view">
					<view class="class-item" :id="'item' + index" v-for="(item, index) in classList" :key="index">
						<view class="item-title">
							<text>{{ item.categoryName }}</text>
						</view>
						<view class="item-container">
							<view class="thumb-box" v-for="(item1, index1) in item.children" :key="index1">
								<image class="item-menu-image" :src="`${BASE_URL}/files/${item1.categoryIcon}`" mode=""></image>
								<view class="item-menu-name">{{ item1.categoryName }}</view>
							</view>
						</view>
					</view>
				</view>
...
import { treeMenu } from '../../utils/index.js';
export default {
	data() {
		return {
			BASE_URL,
	...
	methods: {
		getClassList() {
			this.$u.api.listProductCategory().then(res => {
				this.classList = treeMenu(res.data.data, 'productCategoryId');
				this.getMenuItemTop();
			});
		},
		goDetail(item1) {
