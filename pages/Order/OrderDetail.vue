<template>
	<view class="order-detail">
							...
							<view class="card-box" v-for="(item, inx) in goods.orderCardNoList" :key="inx">
								...
								<view class="card-content">
									...
									<view class="btn-box" v-if="!item.isUsed">
										<u-button type="primary" size="mini" @click="setQr(item, goods)">使用</u-button>
									</view>
								</view>
							</view>
							<view class="card-box">
		...
		<u-modal title="券码" v-model="showQrCode" ref="qrcodeModal" @confirm="getDetail(orderDetail.orderId)">
...
export default {
	...
	methods: {
		...
		changeIsUsed(item) {
			...
			const actives = (list && list.filter(v => !v.isUsed)) || [];
